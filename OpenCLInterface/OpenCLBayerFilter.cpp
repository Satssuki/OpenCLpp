...
/*************************** OpenCLBayerFilterForStream **********************************/

OpenCLBayerFilterForStream::OpenCLBayerFilterForStream(BayerFilterMask mask_type)
{
   switch(mask_type) {
    case BayerFilterMask::CROSS:
      kernel_name = "bayer_image_cross_mask";
  ...
  kparams = mem_balance = NULL;
  //source_filename = "bayer.cl";
  source = "__kernel void  bayer(__global float* input, __global float* output, __constant uchar * params, __global float* balance) \n"
"{\n"
"  int width_output = get_global_size(0); \n"
  "int width = get_global_size(0) + 2; \n"
...
void OpenCLBayerFilterForStream::copyDataToGPUStream()
{
  cl_int err;
...
void OpenCLBayerFilterForStream::setKernelArgsForStream()
{
  //TODO: Release memory
  cl_int err;
...
void OpenCLBayerFilterForStream::setParams(const OpenCLAlgorithmParams& params)
{
  try
  {
    setParams(dynamic_cast<const OpenCLBayerFilterParams&>(params));
  ...
  {
    throw OpenCLAlgorithmException("Wrong params");
  }
}

void OpenCLBayerFilterForStream::setParams(const OpenCLBayerFilterParams& params)
{
  this->params = params;
